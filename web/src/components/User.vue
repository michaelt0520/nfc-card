    ...
    <table class="min-w-full bg-white">
      ...
      <tbody>
        <tr v-for="user in users" :key="user.id">
          ...
          <td class="text-left py-3 px-4 hover:text-blue-500">{{ user.username }}</td>
          <td class="text-left py-3 px-4 hover:text-blue-500">{{ user.type }}</td>
          <td class="text-left py-3 px-4 hover:text-blue-500">{{ user.company_id }}</td>
        </tr>
      </tbody>
    </table>
  </div>
...
export default {
  ...
  methods: {
    ...mapActions('users', ['getListUsers']),
  },

  created() {
    if (!(this.users.length > 1)) this.getListUsers()
  }
}
</script>
